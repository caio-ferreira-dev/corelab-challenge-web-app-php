...
export default function Dashboard() {
    ...
    function renderList(listType: string) {
        ...
        const favoriteFilter = data.filter(note => note.favorite).length >= 1 ? true : false
        const otherFilter =  data.filter(note => !note.favorite).length >= 1 ? true : false
        const renderFavorite = renderSearch ? data.filter(note => (note.name).toLowerCase().includes(query.toLowerCase()) && note.favorite).length >= 1 : favoriteFilter
        const renderOther = renderSearch ? data.filter(note => (note.name).toLowerCase().includes(query.toLowerCase()) && !note.favorite).length >= 1: otherFilter
